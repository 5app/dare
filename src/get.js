...
function buildQuery(opts, accept, reject) {
	...
	let table_alias_handler = opts.table_alias_handler || ((table) => table);
	...
	// Join
	let join_handler = opts.join_handler || (() => {});
	let join = [];

	for (let join_ref in opts.join) {
		// Get the condition
		let join_condition = join_handler(join_ref, opts.join[join_ref]);
