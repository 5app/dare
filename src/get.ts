...
import group_concat from './utils/group_concat.ts';
import field_format from './utils/field_format.ts';
import orderbyUnwrap from './utils/orderby_unwrap.ts';
import SQL, {Sql, raw, join, empty} from 'sql-template-tag';
import type {default as Dare, QueryOptions} from './index.ts';

function optionalJoin(arr: Sql[], joiner: string, prefix: string): Sql {
	if (!arr.length) {
		return empty;
	} else {
...
/**
 * SQL Query Definition, as returned by buildQuery
 */
 ...
 * @param opts - Options for generating the SQL statement
...
function traverse(
	...
	const sql_filter: Sql[] = [];

	/**
	 * Fields
	 */
	const fields: (FieldDefinition | Sql)[] = [];
	...
	// SQL GroupBy
	const groupby: FieldDefinition[] = [];

	// SQL GroupBy
	const orderby: FieldDefinition[] = [];
	...
	if (item.groupby) {
		...
		groupby.push(
			...(item.groupby as string[]).map(field =>
	...
	if (item.orderby) {
		// Either an empty groupby
		const a = (item.orderby as string[]).map(entry => {
