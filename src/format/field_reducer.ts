import unwrapField from '../utils/unwrap_field.ts';
import checkLabel from '../utils/validate_label.ts';
import checkKey from '../utils/validate_field.ts';
import DareError from '../utils//error.ts';
import fieldRelativePath from '../utils/field_relative.ts';
import getFieldAttributes from '../utils/field_attributes.ts';
import jsonParse from '../utils/JSONparse.ts';
import type {default as Dare, Schema} from '../index.ts';

/**
 * Return a reducer function deriving local props and nested props
 * @param opts - Options
 * @param opts.field_alias_path - Current address path of the resource
 * @param opts.extract - Function for handling the extraction of content
 * @param opts.table_schema - Table schema/Data model
 * @param opts.dareInstance - Instance of Dare which is calling this
 * @returns Fields Reducer function
 */
 ...
 *
 * @param opts - Object
 * @param opts.field - Field expression
 * @param opts.label - Optional label, or null
 * @param opts.fieldsArray - The current constructed array of fields
 * @param opts.originalArray - The original fields array as requested
 * @param opts.field_alias_path - Current address path of the resource
 * @param opts.extract - Function for handling the extraction of content
 * @param opts.table_schema - Schema of the current table
 * @param opts.dareInstance - An instance of the current Dare object
 * @param opts.initialCall - Is this the initial call to fieldMapping?
 * @returns The augemented field expression
 */
 ...
 * @param a - Array
 * @param b - Array
 * @returns Reduce array, with matches removed
 */
 ...
 * @param requestPath - Request Path (incl. field)
 * @returns Target path
 */
