...
import type {default as Dare, QueryOptions, Schema} from '../index.ts';

/**
 * Join Handler
 * Obtain the table join conditions which says how two tables reference one another
 * @param join_object - The object being joined
 * @param root_object - The root object, for which we want the join_object too attach
 * @param dareInstance - Dare Instance
 * @returns An updated join_object with new join_conditions attached
 */
export default function joinHandler(
	join_object: QueryOptions,
	root_object: QueryOptions,
	dareInstance: Dare
): QueryOptions | null {
 ...
 * @param object - Object
 * @param object.rootModel - Root model name
 * @param object.linkTable - Link model name
 * @param object.joinModel - Target/join model name
 * @param object.models - Model Options
 * @param object.join_object - Passthrough original join options
 * @param object.dareInstance - Dare Instance
 * @returns Join definition to return
 */
...
function invert_links(
	tableSchema: Schema | undefined,
	joinTable: string,
	dareInstance: Dare
) {
	return links(tableSchema, joinTable, dareInstance, true);
}
