...
import getFieldAttributes from '../../src/utils/field_attributes.ts';
	...
	describe('POST', () => {
		it('should insert default values into post operations', async () => {
			...
			assert(
				typeof sql === 'string'
					? sql.includes('`status`')
					: sql.indexOf('`status`') !== -1
			);
			assert(
				typeof values === 'string'
					? values.includes('active')
					: values.indexOf('active') !== -1
			);
		});
		...
		it('should be overrideable', async () => {
			...
			assert(
				typeof values === 'string'
					? values.includes('overridden')
					: values.indexOf('overridden') !== -1
			);
		});
		...
		['get', 'patch', 'del'].forEach(method => {
			...
			['filter', 'join'].forEach(condition => {
				it(`should be overideable within a dare.${method}() call '${condition}'`, async () => {
					...
					assert(
						typeof values === 'string'
							? values.includes('boom')
							: values.indexOf('boom') !== -1
					);
				});
